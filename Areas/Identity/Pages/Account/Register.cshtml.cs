...
namespace Brunchie.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        private BrunchieUser CreateUser()
        {
            try
            {
                return Activator.CreateInstance<BrunchieUser>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(BrunchieUser)}'. " +
                    $"Ensure that '{nameof(BrunchieUser)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<BrunchieUser> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<BrunchieUser>)_userStore;
        }
    }
}
